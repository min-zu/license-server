  ...
  try {
    ...
    for(let i = 0; i < filteredRows.length; i++) {
      ...
      if (errorMessages.length > 0) {
        ...
        await query("INSERT INTO license_log (action_date, hardware_serial, user, ip, action, `desc`, action_type, customer) VALUES (now(), ?, ?, ?, ?, ?, ?, ?)", [trimmedSerial, session?.user?.name + '(' + session?.user?.id + ')', clientIp, "fail", "파일 업로드 실패(" + errorMessages.join(', ') + ")", "license", customer]);
        continue;
      }
      ...
      */
      ...
      if(result.affectedRows > 0) {
        if(trimmedHardwareCode === '') {
          await query("INSERT INTO license_log (action_date, hardware_serial, user, ip, action, `desc`, action_type, customer) VALUES (now(), ?, ?, ?, ?, ?, ?, ?)", [trimmedSerial, session?.user?.name + '(' + session?.user?.id + ')', clientIp, "success", "파일 업로드 완료(미발급)", "license", customer]);
          await query(`INSERT INTO log_detail SELECT *, NOW() FROM license WHERE hardware_serial = ?;`, [trimmedSerial]);
        }else{
          await query("INSERT INTO license_log (action_date, hardware_serial, user, ip, action, `desc`, action_type, customer) VALUES (now(), ?, ?, ?, ?, ?, ?, ?)", [trimmedSerial, session?.user?.name + '(' + session?.user?.id + ')', clientIp, "success", "파일 업로드 완료(수동)", "license", customer]);
          await query(`INSERT INTO log_detail SELECT *, NOW() FROM license WHERE hardware_serial = ?;`, [trimmedSerial]);
        }
      }
    }
